#!/usr/bin/env python3
"""
SOL Scalping Strategy on Bybit
...
if not API_KEY or not API_SECRET:
    raise ValueError("API credentials not set in .env")
...
RSI_PERIOD = 14
VOLUME_SPIKE_FACTOR = 1.5  # 50% above 20-period average
...
# Initialize exchange
exchange = HTTP(api_key=API_KEY, api_secret=API_SECRET)
...
def get_current_price():
    """Fetch the latest price of SOLUSDT."""
...
def is_volume_spike(df):
    """Detect volume spike (current > 1.5x 20-period average)."""
...
def calculate_position_size(balance, current_price, stop_loss_price):
    """Calculate position size risking 1% of account."""
    ...
    # Adjust to Bybit's minimum quantity and step size (simplified)
...
def main():
    """Main trading loop."""
    daily_loss = 0.0
    logging.info("Starting SOL scalping strategy")
    print("SOL Scalping Strategy Started - Check sol_scalping.log for details")

    while True:
        if not is_trading_time():
            logging.info("Outside trading hours, sleeping for 60 seconds")
            time.sleep(60)
            continue
