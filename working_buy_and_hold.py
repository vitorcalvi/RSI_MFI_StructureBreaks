            ...
            if not be_moved:
                if (side == "Buy" and price >= be_trigger) or (side == "Sell" and price <= be_trigger):
                    ...
                    if result.get('retCode') == 0:
                        print(f"\n{G}✅ Break-even reached! Stop moved to ${entry:,.2f}{RST}")
                        be_moved = True
                        current_stop = entry
                ...
                if side == "Buy":
                    ...
                    if new_stop > current_stop:
                        ...
                        if result.get('retCode') == 0:
                            ...
                            print(f"{G}📈 Trailing stop: ${new_stop:,.2f}{RST}")
                else:
...
def wait_for_limit_fill(exchange, linear, order_id, limit_price, side, timeout=None):
    ...
    print(f"{Y}⏰ Monitoring indefinitely until filled (Ctrl+C to cancel){RST}")
    ...
    while time.time() - start < timeout:
        ...
        if orders.get('retCode') == 0 and not orders['result']['list']:
            print(f"{G}✅ Limit order filled!{RST}")
        ...
        if ticker.get('retCode') == 0:
            ...
            if elapsed >= (adjustment_count + 1) * adjustment_interval and adjustment_count < 4:  # Max 4 adjustments
                ...
                print(f"{Y}📈 Adjusting limit price: ${current_limit:.4f} → ${new_limit:.4f} (adjustment #{adjustment_count}){RST}")
                ...
                if cancel_result.get('retCode') == 0:
                    ...
                    if orders.get('result', {}).get('list') and len(orders['result']['list']) > 0:
                        ...
                        print(f"{R}❌ Could not retrieve order quantity{RST}")
                        return False, None
                else:
                    print(f"{R}❌ Failed to cancel order for adjustment: {cancel_result.get('retMsg')}{RST}")
            ...
            if (side == "Buy" and current_market > current_limit * 1.01) or \
               (side == "Sell" and current_market < current_limit * 0.99):
                print(f"{Y}⚡ Price moved >1% from limit! Converting to market order...{RST}")
                ...
                if cancel_result.get('retCode') == 0:
                    if orders.get('result', {}).get('list') and len(orders['result']['list']) > 0:
                        ...
                        if market_order.get('retCode') == 0:
                            print(f"{G}✅ Market order executed at ~${current_market:.4f}{RST}")
                            time.sleep(1)
                            return True, current_market
                        else:
                            print(f"{R}❌ Market order failed: {market_order.get('retMsg')}{RST}")
                            return False, None
        
        time.sleep(5)
    
    # Final attempt to cancel if timeout
    print(f"{R}❌ Limit order not filled after {timeout//60} minutes{RST}")
    cancel_result = exchange.cancel_order(
        category="linear",
        symbol=linear,
        orderId=order_id
...
def place_trade(direction, symbol, stop_pct=None, rr=None, tp_pct=None, trail_pct=None, order_type="Market", limit_price=None):
    ...
    if atr:
        ...
        print(f"{B}📊 ATR: ${atr:.2f} ({atr_pct*100:.2f}% of price){RST}")
    else:
    ...
    # Display
    print(f"\n{'━' * 60}")
    print(f"{BOLD}🎯 {side.upper()} {symbol}{RST} @ ${entry:,.4f}")
    ...
    try:
        ...
        for i in range(30):  # Wait up to 30 seconds
            ...
            if pos.get('retCode') == 0 and pos['result']['list']:
                if float(pos['result']['list'][0]['size']) > 0:
                    ...
                    print(f"{G}✅ Position created at ${actual_entry:.4f}{RST}")
                    break
            time.sleep(1)
        ...
        if result.get('retCode') == 0:
            print(f"{G}✅ Stops set successfully{RST}")
            ...
            print(f"{R}❌ Failed to set stops: {result.get('retMsg')}{RST}")
        
        # Summary
        print(f"\n{G}{'━' * 60}{RST}")
        print(f"{G}{BOLD}✅ Trade Executed!{RST}")
        print(f"{G}{'━' * 60}{RST}")
        print(f"📊 {side} {qty} {symbol} @ ${actual_entry:,.4f}")
        print(f"🎯 Target: {tp_sign}{tp_pct*100:.1f}% @ ${tp:,.2f} ({G}+${net_profit:,.2f} net{RST})")
        print(f"🛑 Stop: {sl_sign}{stop_pct*100:.1f}% @ ${sl:,.2f} ({R}-${net_loss:,.2f} net{RST})")
        print(f"🔐 BE @ ${be_trigger:,.2f} → ${actual_entry:,.2f}")
        print(f"{G}{'━' * 60}{RST}")
