...
class EthHFTBacktester:
    ...
    def load_data(self):
        df = pd.read_csv(self.data_path, parse_dates=['timestamp'], index_col='timestamp', usecols=['timestamp','open','high','low','close','volume'])
        df = df.resample(self.timeframe).agg({'open':'first','high':'max','low':'min','close':'last','volume':'sum'}).dropna()
        self.data = df
    ...
    def open_position(self, side, price, ts):
        ...
        if side == 'BUY': stop_level = window['low'].min() - price*(self.structure_buffer_pct/100)
        else: stop_level = window['high'].max() + price*(self.structure_buffer_pct/100)
        ...
        self.balance -= price*size*self.entry_fee_pct
        return {'side':side,'entry':price,'stop':stop_level,'tp':tp,'size':size,'open_ts':ts}
    ...
    def manage_position(self, pos, price, ts):
        ...
        if (pos['side']=='BUY'and price<=pos['stop'])or(pos['side']=='SELL'and price>=pos['stop']): self.close_position(pos,price,ts,'Stop Loss'); return True
        if (pos['side']=='BUY'and price>=pos['tp'])or(pos['side']=='SELL'and price<=pos['tp']): self.close_position(pos,price,ts,'Take Profit'); return True
    ...
    def close_position(self, pos, price, ts, reason):
        size=pos['size']; pnl=(price-pos['entry'])*size if pos['side']=='BUY' else (pos['entry']-price)*size
        exit_cost=price*size*self.exit_fee_pct; net_pnl=pnl-exit_cost
        self.balance+=net_pnl
        self.trades.append({'pnl':net_pnl,'close_ts':ts})

    def evaluate(self):
        df=pd.DataFrame(self.trades); total_pnl=df['pnl'].sum(); return total_pnl/self.initial_balance*100

    def report(self):
        if not self.trades: print("No trades executed."); return
        df=pd.DataFrame(self.trades)
        total_ret=df['pnl'].sum()/self.initial_balance*100; win_rate=len(df[df['pnl']>0])/len(df)*100
        print(f"Final Balance: {self.balance:.2f}")
        print(f"Total Return: {total_ret:.2f}% | Win Rate: {win_rate:.1f}% | Trades: {len(df)}")

# Randomized tuner with tqdm progress bar
...
def tune(data_path, max_trials=200, seed=42):
    random.seed(seed)
    grid={
        'rsi_length':[3,5,7],'mfi_length':[3,5,7],'oversold':[30,35,40,45],
        'overbought':[55,60,65,70],'structure_lookback':[20,60,120],
        'structure_buffer_pct':[0.1,0.2,0.3],'fixed_risk_pct':[0.005,0.01,0.02],
        'trailing_stop_pct':[0.005,0.008,0.01],'profit_lock_threshold':[2.0,3.0,4.0],
        'reward_ratio':[3.0,4.0,5.0]
    }
    keys=list(grid.keys())
    best={'params':None,'return':-np.inf}
    for _ in tqdm(range(max_trials), desc='Tuning Params'):
        params={k:random.choice(grid[k]) for k in keys}
        params['entry_fee_pct']=0.00055; params['exit_fee_pct']=0.00055
...
if __name__=='__main__':
    ...
    bt=EthHFTBacktester('_data/ETHUSDT_1_7d_20250731_071705.csv',best['params'])
    bt.load_data(); bt.compute_indicators(); bt.trades.clear(); bt.balance=bt.initial_balance
    pos=None
    for ts,row in bt.data.iterrows():
        price=row['close']
        if pos is None:
            if row['rsi']<bt.oversold and row['mfi']<bt.oversold: pos=bt.open_position('BUY',price,ts)
            elif row['rsi']>bt.overbought and row['mfi']>bt.overbought: pos=bt.open_position('SELL',price,ts)
        else:
            if bt.manage_position(pos,price,ts): pos=None
    if pos: bt.close_position(pos,bt.data['close'].iloc[-1],bt.data.index[-1],'End')

    bt.report()

    eq=[bt.initial_balance]; times=[bt.data.index[0]]
    for trade in bt.trades: eq.append(eq[-1]+trade['pnl']); times.append(trade['close_ts'])
    plt.figure(figsize=(10,5)); plt.plot(times,eq); plt.title('Equity Curve with Tuned Strategy'); plt.xlabel('Time'); plt.ylabel('Balance'); plt.grid(True); plt.show()
